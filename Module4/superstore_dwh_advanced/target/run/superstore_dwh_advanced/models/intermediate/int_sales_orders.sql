  ...
  create view "superstore"."public_dw_intermediate"."int_sales_orders__dbt_tmp"
...
LEFT JOIN "superstore"."dw_test"."customer_dim" AS c ON f.cust_id = c.cust_id
LEFT JOIN "superstore"."dw_test"."product_dim" AS p ON f.prod_id = p.prod_id
LEFT JOIN "superstore"."dw_test"."shipping_dim" AS s ON f.ship_id = s.ship_id
LEFT JOIN "superstore"."dw_test"."geo_dim" AS g ON f.geo_id = g.geo_id
-- ИСПРАВЛЕНО: Добавляем псевдонимы, так как календарь используется дважды
LEFT JOIN "superstore"."dw_test"."calendar_dim" AS cal_order ON f.order_date_id = cal_order.dateid
LEFT JOIN "superstore"."dw_test"."calendar_dim" AS cal_ship ON f.ship_date_id = cal_ship.dateid
  );